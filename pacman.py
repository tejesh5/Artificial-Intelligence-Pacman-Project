...
def readCommand(argv):
    ...
    parser.add_option('--csvFile', dest='csvFile', 
                      help=default('Name of csv file to store results'), default='results.csv')
    parser.add_option('--clearCSV', action='store_true', dest='clearCSV', 
                      help=default('For overwriting the existing csv file'), default=False)
    ...
    args['clearCSV'] = options.clearCSV
    args['csvFile'] = options.csvFile
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30, clearCSV=False, csvFile='result.csv'):
    import __main__
    ...
    if clearCSV:
        # delete the csv file
        os.system(f"rm -r {csvFile}")

    # to store the results for this run
    if 'saveResults' in dir(pacman):
        pacman.saveResults(csvFile)
